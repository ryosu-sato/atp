...
all: atp_batch.cmxa atp_batch.cma

atp_batch.cmxa: atp_batch.cmx
	ocamlopt -a -o $@ $^

atp_batch.cma: atp_batch.cmo
	ocamlc -a -o $@ $^

install: atp_batch.cma atp_batch.cmxa
	ocamlfind remove atp
	ocamlfind install atp META atp_batch.a atp_batch.cmi atp_batch.cma atp_batch.cmxa
...
bytecode: example.ml atp_batch.cmo
	ocamlfind ocamlc -package num -pp "camlp5o ./Quotexpander.cmo" -o example atp_batch.cmo example.ml

# Alternatively, produce native code

compiled: example.ml atp_batch.cmx
	ocamlfind ocamlopt -package num -pp "camlp5o ./Quotexpander.cmo" -o example atp_batch.cmx example.ml

# Make the appropriate object for the main body of code

atp_batch.cmx: Quotexpander.cmo atp_batch.ml
	ocamlfind ocamlopt -package num -pp "camlp5o ./Quotexpander.cmo" -w -a-x -c atp_batch.ml

atp_batch.cmo: Quotexpander.cmo atp_batch.ml
	ocamlfind ocamlc -package num -pp "camlp5o ./Quotexpander.cmo" -w -a-x -c atp_batch.ml

# Make the camlp4 or camlp5 quotation expander

Quotexpander.cmo: Quotexpander.ml
	ocamlfind ocamlc -package camlp5 -c Quotexpander.ml
